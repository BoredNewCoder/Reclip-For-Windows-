...
secondary_types: []
confidence: 0.731
detected_at: 2026-05-28T22:25:29.706Z
signals:
...
- Framework(s): Flask 3.1.3, yt-dlp 2026.3.17
- Package manager: pip (venv at `venv/`)
- Runtime / deployment target: local HTTP server, default port 8899 (`HOST`/`PORT` env vars)

### Commands
- Install: `pip install -r requirements.txt`
- Build: N/A (no build step)
- Test (all): TODO (no test suite)
- Test (single file): TODO
- Lint: TODO
- Typecheck: TODO
- Run locally: `python app.py` or `reclip.bat` on Windows (auto-creates venv, checks ffmpeg)

### Layout
- Source lives in: `app.py` (single-file backend), `templates/index.html` (frontend HTML), `static/favicon.svg`
- Tests live in: none
- Do not modify: `venv/` (generated), `downloads/` (runtime output), `cookies.txt` (runtime state)

### Conventions specific to this repo
- Naming: snake_case throughout (Python stdlib style)
- Import style: stdlib first, then third-party (Flask, yt-dlp)
- Error handling pattern: `subprocess.run` + `returncode` check; exceptions caught → `jsonify({"error": ...})` + HTTP 4xx/5xx
- Testing pattern and framework: none configured

### Forbidden
- Do not import or use async frameworks (asyncio, aiohttp) — current threading model is intentional.
- Do not shell out to `yt-dlp` binary directly; always use `[sys.executable, "-m", "yt_dlp"]` so the venv copy is used.
